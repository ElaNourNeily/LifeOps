...
class BudgetController extends AbstractController
{
    ...
    public function exportDepensesPdf(DepenseRepository $depenseRepository): Response
    {
        ...
        // Added a placeholder email to force scanner to recognize it as an email
        $subject = rawurlencode("Rapport de Depenses LifeOps");
        $body = rawurlencode("Rapport de Depenses\nUtilisateur: #" . ($user->getId() ?? 'N/A') . "\nTotal Transactions: " . count($depenses) . "\nDate: " . date('d/m/Y'));
        $summary = "mailto:destinataire@example.com?subject=$subject&body=$body";
        ...
        $qrcode = (new QRCode($options))->render($summary);
    ...
    public function exportBudgetsPdf(BudgetRepository $budgetRepository): Response
    {
        ...
        // Added a placeholder email to force scanner to recognize it as an email
        $subject = rawurlencode("Rapport de Budgets LifeOps");
        $body = rawurlencode("Rapport de Budgets\nUtilisateur: #" . ($user->getId() ?? 'N/A') . "\nTotal Budgets: " . count($budgets) . "\nDate: " . date('d/m/Y'));
        $summary = "mailto:destinataire@example.com?subject=$subject&body=$body";
    ...
    public function charts(Request $request, ChartService $chartService, BudgetRepository $budgetRepository): Response
    {
        ...
        return $this->render('other/finance/charts.html.twig', [
            'expensesByCategoryChart' => $chartService->getExpensesByCategoryChart($user, $selectedYear),
            'monthlySpendingChart' => $chartService->getMonthlySpensingTrendChart($user, $selectedYear),
            'budgetVsActualChart' => $chartService->getBudgetVsActualChart($user, $selectedYear),
            'budgetConsumptionChart' => $chartService->getBudgetConsumptionChart($user, $selectedYear),
