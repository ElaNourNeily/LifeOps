...
class ActiviteController extends AbstractController
{
    ...
    {
        $user = $this->getUser();
        $dateString = $request->query->get('date', 'today');
        try {
            $date = new \DateTime($dateString);
        } catch (\Exception $e) {
            $date = new \DateTime('today');
        }
        $date->setTime(0, 0, 0); // Normalize date
        $view = $request->query->get('view', 'week');

        if ($view === 'day') {
            $startOfWeek = clone $date;
            $endOfWeek = (clone $date)->setTime(23, 59, 59);
        } else {
        ...
        if (count($plannings) > 0) {
            ...
            foreach ($plannings as $p) {
                ...
                // Also check activity times to ensure they are visible
                foreach ($p->getActivites() as $act) {
                    if ($act->getHeureDebutEstimee()) {
                        $hActStart = (int) $act->getHeureDebutEstimee()->format('G');
                        if ($hActStart < $minStart) $minStart = $hActStart;
                    }
                    if ($act->getHeureFinEstimee()) {
                        $hActEnd = (int) $act->getHeureFinEstimee()->format('G');
                        if ($hActEnd > $maxEnd) $maxEnd = $hActEnd;
                    }
                }
            }
            $gridStart = $minStart;
